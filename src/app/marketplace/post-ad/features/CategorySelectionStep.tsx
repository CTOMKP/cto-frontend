...
const TAB_PANEL_CLASS =
  'mt-14 border-[0.2px] border-white/20 rounded-lg p-6 w-[664px] mx-auto';
...
function CategorySubcategoryForm({
  ...
  onToggleSubcategoryInput,
  onCreatePost,
}: {
  selectedCategory: string | null;
  selectedSubcategory: string;
  subcategoryInputOpen: boolean;
  selectedCategoryData: Category | undefined;
  onCategorySelect: (categoryId: string) => void;
  onSubcategorySelect: (subcategory: string) => void;
  onSubcategoryInputChange: (value: string) => void;
  onToggleSubcategoryInput: (open: boolean) => void;
  onCreatePost: () => void;
}) {
  return (
    <>
        ...
        <div className="mb-6">
          ...
          {subcategoryInputOpen ? (
            <div>
              <div className="relative flex items-center justify-between">
                <Input
                  ...
                  onChange={(e) => onSubcategoryInputChange(e.target.value)}
                  className="w-full bg-[#1A1A1A] border-[#404040] text-white placeholder:text-[#606060] pr-10"
                />
                <button
                  onClick={() => onToggleSubcategoryInput(false)}
            ...
            <div className="relative">
              <button
                onClick={() => onToggleSubcategoryInput(true)}
...
export default function CategorySelectionStep({ onNext }: CategorySelectionStepProps) {
  ...
  const tabValue = postType === 'OFFERING' ? 'offering' : 'looking-for';
  ...
  const formProps = {
    ...
    onSubcategoryInputChange: setSelectedSubcategory,
    onToggleSubcategoryInput: setSubcategoryInputOpen,
  ...
  return (
    <div className="">
      <div className="max-w-4xl mx-auto my-17 border-[0.2px] border-white/20 rounded-lg p-4">
        <Tabs
          value={tabValue}
          onValueChange={(v) => setPostType(v === 'offering' ? 'OFFERING' : 'LOOKING_FOR')}
        >
          ...
          <TabsContent value="looking-for" className={TAB_PANEL_CLASS}>
            <CategorySubcategoryForm {...formProps} />
          </TabsContent>

          <TabsContent value="offering" className={TAB_PANEL_CLASS}>
            <CategorySubcategoryForm {...formProps} />
          </TabsContent>
        </Tabs>
      </div>
