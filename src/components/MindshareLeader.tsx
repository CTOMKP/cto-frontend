...
export default function MindshareLeader() {
  return (
    <div className='flex items-center'>
      <span className='flex items-center mr-2'><p className='text-[#0042ED]'>Mindshare leader</p> <Image src="/emoji-icons/speaker.svg" alt="/gaining-traction" width={15.5} height={15.5}/></span>

        {mockData.map((item, index) => (
            <div key={index} className={`flex items-center gap-1 ${index === 0 ? '' : 'ml-3'}`}>
                <span className='text-[#393939]'>#{index + 1}</span>
                <Image src="/default-trending-coin-img.png" alt="default-trending-coin-img" width={16} height={16} />
